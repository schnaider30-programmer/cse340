...
async function updateInventory(classification_id, inv_id, inv_make, inv_model, inv_description, inv_image, inv_thumbnail, inv_price, inv_year, inv_miles, inv_color) {
  ...
  try {
    const result = await pool.query(sql, [classification_id, inv_make, inv_model, inv_description, inv_image, inv_thumbnail, inv_price, inv_year, inv_miles, inv_color, inv_id])
    return result.rows[0]
  } catch (error) {
...
async function recordMaintenance(maint_type, maint_desc, maint_cost, maint_responsible, maint_date, maint_next_due_date, inv_id) {
  try {
    const sql = "INSERT INTO maintenance(maint_type, maint_desc, maint_cost, maint_responsible, maint_date, maint_next_due_date, inv_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *;"
    const result = await pool.query(sql, [maint_type, maint_desc, maint_cost, maint_responsible, maint_date, maint_next_due_date, inv_id])
    return result.rows[0];
  } catch (error) {
...
async function getMaintenanceByInventoryId(inv_id) {
  try {
    const sql = "SELECT i.inv_make, i.inv_model, i.inv_year, m.* FROM inventory AS i JOIN maintenance as m ON i.inv_id = m.inv_id WHERE m.inv_id = $1;"
    const result = await pool.query(sql, [inv_id])
    return result.rows;
  } catch (error) {
